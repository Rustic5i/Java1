...
//   где по получателю можно получить список зарплат, которые были ему отправлены.
    ...
    public static class MailService<T> implements Consumer<IntefContent<T>> {
        ...
        @Override
        public void accept(IntefContent<T> content) {
            if (mailBox.containsKey(content.getTo())) {
                mailBox.get(content.getTo()).add(content.getContent());
            } else {
                mailBox.put(content.getTo(), new ArrayList<>(Arrays.asList(content.getContent())));
            }
        }
    }


    public static class Salary implements IntefContent<Integer> {
        private String compani;
        private String to;
        private Integer content;

        public Salary(String compani, String to, int content) {
    ...
    public static class MailMessage implements IntefContent<String> {
        private String from;
        private String to;
